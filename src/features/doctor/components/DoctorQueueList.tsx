  ...
  if (loading) {
    return (
      <div className="p-8 text-center text-slate-400">
        <p className="text-sm">Loading doctor patient queue...</p>
      </div>
    );
  }
  ...
  if (appointments.length === 0) {
    return (
      <div className="p-12 text-center border-2 border-dashed border-slate-800 rounded-xl bg-slate-900/40">
        ...
        <p className="text-base font-bold text-slate-200">No Patients Awaiting Doctor Assessment</p>
    ...
    <div className="space-y-4">
      {appointments.map((item) => {
        ...
        return (
          <div
            key={item.id}
            className="glass-card p-5 rounded-xl border border-purple-500/20 hover:border-purple-500/40 transition flex flex-col md:flex-row items-start md:items-center justify-between gap-4"
          >
