            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-600 dark:text-slate-400 font-semibold">
                  Scheduled for: <strong className="text-slate-900 dark:text-slate-200">{formatDate(item.scheduled_at)}</strong>
                </span>
              </div>
            ...
            {item.status !== 'rejected' && item.status !== 'cancelled' && (
              <div className="pt-2">
                ...
                <div className="grid grid-cols-5 gap-1.5 text-center text-[10px] font-extrabold">
                  ...
                  <div className={`py-2 rounded-lg transition ${step >= 2 ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-950 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-500/40' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500'}`}>
                    2. Approved
                  </div>
                  ...
                  <div className={`py-2 rounded-lg transition ${step >= 5 ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-950 dark:text-blue-300 border border-blue-300 dark:border-blue-500/40' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500'}`}>
                    5. Completed
                  </div>
                </div>
              </div>
