...
export const Card: React.FC<CardProps> = ({ children, title, subtitle, action, className = '' }) => {
  return (
    <div className={`rounded-xl p-6 shadow-xs hover:shadow-sm transition-all duration-200 border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900/90 ${className}`}>
      {(title || action) && (
